...
Before any restore mutation, the latest pointer and snapshot are treated as untrusted input. Selected-network snapshots are valid only when the captured baseline category is Public, because Safe Repair only changes Public to Private. Restore also refuses to override an interface that is currently DomainAuthenticated, even when the historical snapshot baseline was valid. SMB1 legacy snapshots are valid only when the captured baseline is exactly Disabled; pending, superseded, partially installed, payload-removed, enabled, or unknown states are not mutated because this tool cannot guarantee an exact rollback. Firewall-only restore snapshots must contain at least one managed File and Printer Sharing rule within the Domain/Private/Any Safe Repair profile scope; Public-only firewall state is rejected, including inside combined Safe Repair snapshots. Registry snapshot capture uses strict registry reads so an access/read failure cannot be mistaken for an absent value. Restore writes are followed by state read-back for registry values, firewall rules, network profiles, Windows features, and services; a command returning without error is not sufficient for success.
...
Safe Repair preflights firewall, selected-network, and Network Discovery actions before creating a snapshot. Network Discovery snapshots only the discovery services that are not already Running. Combined Safe Repair follows the same mutation-set rule: Spooler is always captured because it is restarted, Network Discovery services are captured only when not already Running, and firewall state is captured only for rules that actually need Domain/Private repair. Legacy/high-risk compatibility actions use the same lifecycle principle: already-satisfied registry/SMB1 targets do not create a new snapshot, RPC Named Pipes snapshots only targets that still require mutation, and an unreadable/`Unknown` SMB1 baseline aborts before mutation because a reliable rollback state is unavailable. If the requested state is already satisfied or the repair cannot start, `Restore latest` is left unchanged. Firewall snapshots include only the rules that actually require mutation.

Temporary Point and Print first checks printer inventory for the requested UNC. If the target is already connected, the action returns before `RISK` confirmation, snapshot creation, or any Point and Print registry change. Temporary Point and Print compatibility uses a managed snapshot as an emergency rollback anchor while protection is lowered. The printer connection is also read back from inventory before the connection attempt may report success. After the original registry state is restored successfully, the temporary snapshot is removed and the previous `Restore latest` pointer is reinstated. If rollback cannot be confirmed, the emergency snapshot is deliberately retained as the latest recovery state.
